# Standard libraries
import os, pickle, re, time, logging, operator,subprocess,warnings, string
...
class Downloader:
    """
    Class for downloading ZTF FITS photometric measurements and entire images from IRSA/IPAC database using metadata from provided CSV file.
    """

    def __init__(self, csv_file: str, path_to_save_fits: str, ptype:str='psfcat', max_workers: int = 5, verbose: bool = False):
        """
        Parameters:
        -----------
        csv_file : str
        ...
        ptype : str
            Product type to be downloaded (default='psfcat').
            Other than 'psfcat' types are 'image' and 'scimrefdiffimg' to download the raw and differencial images as fits.fz files.
        max_workers : int
    ...
    def _build_filename(self, row, ptype):
        ...
        qid = str(row['qid'])
        if ptype == 'image':
            return f"ztf_{filefracday}_{field}_{filtercode}_c{ccdid}_{imgtypecode}.fits.fz"
        elif ptype=='scimrefdiffimg':
            return f"ztf_{filefracday}_{field}_{filtercode}_c{ccdid}_{imgtypecode}_q{qid}_{ptype}.fits.fz"
        else:
            return f"ztf_{filefracday}_{field}_{filtercode}_c{ccdid}_{imgtypecode}_q{qid}_{ptype}.fits"

    def _build_url(self, filefracday, filename, ptype):
        if ptype == 'image':
            link =  f"https://irsa.ipac.caltech.edu/ibe/data/ztf/products/raw/{filefracday[:4]}/{filefracday[4:6]}{filefracday[6:8]}/{filefracday[8:]}/{filename}"
        else:
            link =  f"https://irsa.ipac.caltech.edu/ibe/data/ztf/products/sci/{filefracday[:4]}/{filefracday[4:6]}{filefracday[6:8]}/{filefracday[8:]}/{filename}"
        return link

    def _download_file(self, idx, row, ptype):
        filename = self._build_filename(row, ptype)
        full_path = os.path.join(self.save_path, filename)
    
        # Build URL and download command
        filefracday = str(row['filefracday'])
        url = self._build_url(filefracday, filename, ptype)
    ...
    def download_fits(self,ptype='psfcat'):
        """Downloads missing FITS files in parallel.
           ptype define the product type to be downloaded (default = psfcat).
        """
        self.logger.info("Starting download of FITS files...")
        futures = []
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            for idx, row in self.csv_file.iterrows():
                futures.append(executor.submit(self._download_file, idx, row, ptype))
...
class RotationalAnalysis:
    ...
    def lomb_scargle_fit(self, asteroid: str, filter_code: str = None, shev: str = 'No',legend = False, 
    ...
    @staticmethod
    def per_sample_analysis(asteroid: str, table, nterms: list, P_ref: float,
                           f_min: float, f_max: float, save_path: str,
                           delta_t: float, samples_per_peak: int,
                           n_shuffle: int = None, use_window_function: bool = False):
        """
        Perform per-subsample rotational analysis on a list of subsamples-specific photometric tables.
        """
        rot = {}
        tab = {}
        folded_models = {}
        ...
        for i, (orbit_table, n) in enumerate(zip(table, nterms)):
            label = string.ascii_uppercase[i]
            key = f"{label}"
            filename1 = f"{save_path}_{label}_corrected_by_phase.jpg"
            filename2 = f"{save_path}_{label}_individual_periodogram.jpg"
            filename3 = f"{save_path}_{label}_lightcurve_folded.jpg"
...
class Plots:
    ...
    def general(self, x_column: str = 'jd_lt_corr', y_column: str = 'reduced_mag',
        ...
        # 2) Validate columns
        def _has(col):
            return (hasattr(filter_data, 'colnames') and col in filter_data.colnames) or \
                   (hasattr(filter_data, 'columns') and col in filter_data.columns)
    
        for col, needed in [(x_column, True), (y_column, True),
                            (err_column, show_yerr), (color_column, show_color)]:
            if needed and not _has(col):
                raise ValueError(f"Column '{col}' not found in the filtered data.")
    
        # 3) Convert Time -> JD BEFORE converting to pandas
        if hasattr(filter_data, 'colnames'):  # Astropy Table
        ...
        # 4) Error array (clean non-finite)
        err_array = None
        if show_yerr:
        ...
        # 5) Labels
        labels = {'separation (mas)': 'O-C (mas)'} if (show_color and color_column == 'separation (mas)') else None
    
        # 6) Main scatter (column names only)
        fig = px.scatter(
            filter_data,
            x=x_column,
            y=y_column,
            ...
            labels=labels,
            render_mode="svg"   # force regular 'scatter' (SVG) instead of 'scattergl'
        )
    
        # Error bars (no 'truncated')
        if show_yerr:
        ...
        # 9) Optional gray "previous" points
        if bkg_data:
            fig.add_scatter(
        ...
        # 10) Y-range: use provided limits or compute from data+errors (and reverse for magnitudes)
        if isinstance(ylim, list) and len(ylim) == 2:
            fig.update_yaxes(range=ylim, autorange=False)
        else:
            yvals = np.asarray(filter_data[y_column], dtype=float)
            if show_yerr and err_array is not None:
                ylow = yvals - err_array
                yhigh = yvals + err_array
            else:
                ylow = yvals
                yhigh = yvals
            finite_low = ylow[np.isfinite(ylow)]
            finite_high = yhigh[np.isfinite(yhigh)]
            if finite_low.size and finite_high.size:
                ymin = float(np.nanmin(finite_low))
                ymax = float(np.nanmax(finite_high))
                pad = 0.02 * (ymax - ymin) if np.isfinite(ymax - ymin) else 0.0
                # Magnitudes: invert axis by giving reversed range explicitly
                fig.update_yaxes(range=[ymax + pad, ymin - pad], autorange=False)
            else:
    ...
    def phase_curve(self,popt,fit_label,popt2,fit_label_binned,x_column='phase_angle',
                    ...
                    save_path=None,show_yerr=False,show_color=False,show_fig = False,
                    table=None,ylim=None,bkg=True,bkg_data=False,label=True,width=1500, height=500):
        """
        ...
        ylim: list
            Optional, allow to set ylimits for the plot by providing a list of two values : [ymin,ymax]
            Default=None
        ...
        try:
            ...
            fig.add_scatter(
                ...
                y=mag_fit,
                mode='lines',
                name=fit_label if label else None,
                showlegend=label,
            ...
            fig.add_scatter(
                ...
                y=mag_fit_binned,
                mode='lines',
                name=fit_label_binned if label else None,
                showlegend=label,
    ...
    def periodogram(self,asteroid: str, frequencia: np.ndarray, potencia: np.ndarray, 
        ...
        if filter_code:
            plt.title(f'{asteroid} - {filter_code} - {n_term} terms', fontsize=self.fontsize)
        else:
            plt.title(f'{asteroid} - all - {n_term} terms', fontsize=self.fontsize)
        plt.xlabel('Frequency (cycles/day)', fontsize=self.fontsize)
        plt.ylabel('Power LS', fontsize=self.fontsize)
        ...
        if peaks:
            ...
            for i, idx in enumerate(top_indices):
                ...
                plt.text(x + (fig_width / 300), y - (fig_height / 300), str(i + 1),
                         fontsize=self.fontsize - 2, color='black', ha='center', va='bottom')
                print(f"{i+1}: Period = {24/x:.6f} hours, Power = {y:.6f}")
        
        if legend == True:
            plt.legend(fontsize=self.fontsize - 5, loc='upper left', bbox_to_anchor=(1, 1))
    ...
    @staticmethod
    def plot_folded_lightcurve_with_residuals(df, save_path=None,fontsize=18):
        """
        ...
        """
        fig, (ax1, ax2) = plt.subplots(nrows=2, ncols=1, figsize=(12, 4.75), sharex=True, 
        ...
        ax2.axhline(0, color='gray', lw=1, linestyle='--',zorder=10)
        ax2.plot(df['Rotational phase'], df['residuals'], '.', markersize=2, color='darkred')
        ax2.set_ylabel("Residuals",fontsize=fontsize)
        ax2.set_xlabel("Rotational Phase",fontsize=fontsize)
        ax2.invert_yaxis()
    ...
    @staticmethod
    def plot_all_folded_samples(folded_models, save_path, rot_dict, align_to='min', align_phase=0.5, fontsize=24, bkg=True):
            """
            ...
            align_to : {'min', 'max'} or None
                If 'min' or 'max', align all subsamples based on model min/max.
                If None, no phase alignment applied.
            align_phase : float
                Phase to center the alignment (default = 0.5)
            bkg : bool
                If False, hides plot background and grid. Default is True.
            """
            fig = go.Figure()
            ...
            for i, (orbit_name, df) in enumerate(folded_models.items()):
                ...
                fig.add_trace(go.Scatter(
                    ...
                    mode='markers',
                    name=f"{rot_dict[orbit_name].amplitude:.3f} +/- {rot_dict[orbit_name].std_model_residuals:.3f} mag",
                    marker=dict(color=color, size=6),
                    legendgroup=orbit_name,
                    showlegend=True
                ))
        
                # Sort model for continuous line
                sorted_idx = np.argsort(aligned_model_phase)
                x_sorted = aligned_model_phase[sorted_idx]
                y_sorted = model_mag[sorted_idx]
        
                # Detect wrap points and split
                jumps = np.where(np.diff(x_sorted) < -0.5)[0]
            ...
            # Axis config (borders and ticks)
            xaxis_cfg = dict(
                range=[0, 1],
                title_font=dict(size=fontsize, color="black"),
                tickfont=dict(size=fontsize - 4, color="black"),
                showline=True,
                mirror=True,
                linecolor='black',
                linewidth=1.5,
            ...
            fig.update_layout(
                ...
                plot_bgcolor='white' if bkg else 'rgba(0,0,0,0)',
                paper_bgcolor='white' if bkg else 'rgba(0,0,0,0)',
    ...
    @staticmethod
    def plot_all_periodograms(frequency, power, best_frequency, error, save_path, fontsize=24, bkg=True):
        '''
        ...
        ----------
        frequency : dict
            A dictionary where keys are subsample identifiers, values are frequency arrays.
        
        power : dict
            A dictionary with the same keys as `frequency`, values are corresponding LS power arrays.
        
        best_frequency : float
            The reference frequency (in cycles/day).
        
        error : float
            Error bar in **hours**, used to compute dashed vertical markers around the best period.
        ...
        bkg : bool, optional
            Whether to show background and grid. Default is True.
    
        Returns
        -------
        None
        '''
        palette = px.colors.qualitative.Set1[1:]  # Skip red
    
        fig = go.Figure()
    
        # Add traces for each subsample
        ...
        P_center = 24 / best_frequency  # in hours
        P_plus = P_center + error
        P_minus = P_center - error
        
        f_plus = 24 / P_plus  # back to freq
        f_minus = 24 / P_minus
        
        fig.add_vline(x=f_minus, line_dash="dot", line_color="black", line_width=2)
        fig.add_vline(x=f_plus, line_dash="dot", line_color="black", line_width=2)
    
    
        # Set layout with conditional background/grid
        layout = dict(
            margin=dict(l=50, r=50, t=20, b=50),
            ...
            width=1200,
            height=500,
            xaxis=dict(title_font=dict(size=fontsize), tickfont=dict(size=fontsize)),
            yaxis=dict(title_font=dict(size=fontsize), tickfont=dict(size=fontsize)),
            legend=dict(
        ...
        if not bkg:
            layout.update({
                ...
                'xaxis': dict(
                    ...
                    mirror=True,             # mirror to top
                    linecolor='black',
                    linewidth=1.5,
                    showgrid=False,
                    zeroline=False,
                    title_font=dict(size=fontsize),
                    tickfont=dict(size=fontsize - 4)
                ),
                'yaxis': dict(
                    showline=True,
                    mirror=True,             # mirror to right
                    linecolor='black',
                    linewidth=1.5,
                    showgrid=False,
                    zeroline=False,
        ...
        # Save and show
        fig.write_image(save_path + "_periodogram.png", width=1200, height=300, scale=3)
        fig.show()